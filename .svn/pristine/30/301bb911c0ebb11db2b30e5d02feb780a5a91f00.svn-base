...
public class Animation implements GameObject {
	...
	/**public void resize(int width, int height){
		if(_frameIndex == 4){
			_frames.get(_frameIndex).loadBitmap(R.drawable.jelly_closeeye);
		}else{
			_frames.get(_frameIndex).loadBitmap(R.drawable.jelly);
		}
		_frames.get(_frameIndex).resize(width, height);
	}*/
}
